...
function RoadmapOverview({ roadmap, onStartJourney }: {
  ...
  const phasesArray = Array.isArray(roadmap.phases) 
    ? roadmap.phases 
      ...
      <div className="text-center space-y-4">
        ...
        >
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-gray-300">{phasesArray.length} phases</span>
          </div>
        </motion.div>
      </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                <div className="text-2xl font-bold text-neon-cyan">
                  {phasesArray.reduce((sum, phase) => sum + (phase.nodes?.length || 0), 0)}
                </div>
                <div className="text-sm text-gray-400">Learning Modules</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-neon-pink">
                  {phasesArray.reduce((sum, phase) => 
                    sum + (phase.nodes?.filter(node => node.type === 'project').length || 0), 0
                  )}
                </div>
...
export default function RoadmapPage() {
  ...
  const handleStartJourney = async () => {
    ...
    try {
      // Convert phases to array format if needed
      const phasesArray = Array.isArray(generatedRoadmap.phases)
        ? generatedRoadmap.phases
        : Object.values(generatedRoadmap.phases || {})
      ...
      const response = await fetch('/api/roadmap', {
        ...
        body: JSON.stringify({
          ...
          nodes: phasesArray.flatMap(phase => phase.nodes || []),
          connections: phasesArray.map((phase, index) => ({
            from: index > 0 ? phasesArray[index - 1].id : 'start',
            to: phase.id,
            type: 'sequence'
          }))
        })
